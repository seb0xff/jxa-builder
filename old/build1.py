...
# TODO: change app_name to output_name
# TODO: use tempfile module if possible
# TODO: instead of using ~ and / use os.path.expanduser and os.path.expandvars
# TODO: or use pathlib
# TODO: use pathlib resolve() to parse Libary() calls
# TODO: use pathlib glob to include files
# TODO: add logging
...
try:
  with open(p.join(build_dir, 'locations.json'), 'w') as f:
...
if jxa_config.comp_mode == "app":
  ...
  if jxa_config.app_icon:
    if not p.exists(jxa_config.app_icon):
      print(f'Error, icon file "{jxa_config.app_icon}" not found')
      exit(1)
    try:
      shutil.copy2(
          jxa_config.app_icon,

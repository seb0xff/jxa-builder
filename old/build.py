...
class Literals:
    ...
    comp_mode = Option('compilationMode', '--comp-mode', Literal['app', 'script', 'bundle'])
    deps_comp_mode = Option('dependenciesCompilationMode', '--deps-comp-mode', ['script', 'bundle'])
    deps_install_mode = Option('dependenciesInstallationMode', '--deps-install-mode', ['app', 'user', 'system'])
...
## Parse arguments
parser = argparse.ArgumentParser(description='Javascript for Automation build system')
parser.add_argument(Literals.working_dir.cmd, type=str, default=os.getcwd(), help='Working directory. Place where the project is located. Defaults to the current directory')
parser.add_argument(Literals.comp_mode.cmd, type=str, choices=Literals.comp_mode.vals, help='Whether to compile the main source to a standalone app (e.g. applet/droplet) or a script')
parser.add_argument(Literals.deps_comp_mode.cmd, type=str, choices=Literals.deps_comp_mode.vals, help='Whether dependencies should be compiled as scripts or bundles')
parser.add_argument(Literals.deps_install_mode.cmd, type=str, choices=Literals.deps_install_mode.vals, help='Where to install dependencies. If the main file is compiled to a standalone app one can embed them inside. Otherwise they can be installed for a current user, or system wide')
parser.add_argument(Literals.version.cmd, type=str, help='Version of the project')
parser.add_argument(Literals.main_file.cmd, type=str, help='Main source file')
parser.add_argument(Literals.app_name.cmd, type=str, help='App name, (has effect only when compiling to a standalone app)')
parser.add_argument(Literals.app_icon.cmd, type=str, help='By default the builder searches for "icon.icns" or "<app-name>.icns" in the working directory. If both icons are not found and this option is left empty, the default icon is used (has effect only when compiling to a standalone app)')
parser.add_argument(Literals.debug_info.cmd, action='store_true', help='Whether to add some useful information. (mainly used for testing purposes)')
subparsers = parser.add_subparsers()
install_parser = subparsers.add_parser('install')
install_parser.add_argument(Literals.deps_only.cmd, default=False, action='store_true', help='Whether to install the app and it\'s dependencies or only the dependencies')
...
def manage_outputs(action: Literal['install', 'uninstall'], exit_after: bool = False):
      ...
      if target != dest_file:
        try:
          if p.exists(dest_file):
            shutil.rmtree(dest_file) if p.isdir(dest_file) else os.remove(dest_file)
          if action == 'install':
            os.makedirs(install_path, exist_ok=True)
            shutil.copytree(target, install_path) if p.isdir(target) else shutil.copy(target, install_path)
    ...
    if perm_problem:
      ...
      if action == 'install':
        try:
          ...
          print(f'Error, while copying output directory to the temporary (system) directory: {e}')
          exit(1)
      try:
        process = subprocess.run(['osascript', '-l', 'JavaScript', '-e', jxa_command], text=True)
...
class JxaConfig:
  def __init__(self, path: str):
    ...
    self.comp_mode: Literal['app', 'script', 'bundle'] = ((args.comp_mode if is_arg_provided(Literals.comp_mode.cmd) and isWorkingDir else json_config.get(Literals.comp_mode.json, '')) or 'app').strip()
    self.deps_comp_mode: Literal['script', 'bundle'] = ((args.deps_comp_mode if is_arg_provided(Literals.deps_comp_mode.cmd) and isWorkingDir else json_config.get(Literals.deps_comp_mode.json, '')) or 'script').strip()
    self.deps_install_mode: Literal['app', 'user', 'system'] = ((args.deps_install_mode if is_arg_provided('--dep-install-mode') and isWorkingDir else json_config.get('dependencyInstallationMode', '')) or 'app').strip()
    self.version: str = ((args.version if is_arg_provided('--version') and isWorkingDir else json_config.get('version', '')) or '1.0.0').strip()
    self.main_file: str = ((args.main_file if is_arg_provided('--main-file') and isWorkingDir else json_config.get('main', '')) or 'index.js').strip()
    self.app_name: str = ((args.app_name if is_arg_provided('--app-name') and isWorkingDir else json_config.get('appName', '')) or 'app').strip()
    self.app_icon: str = ((args.app_icon if is_arg_provided('--app-icon') and isWorkingDir else json_config.get('appIcon', '')) or '').strip()
    if not self.app_icon and p.exists(p.join(working_dir, self.app_name + '.icns')):
    ...
    self.install_after: bool = ((args.install_after if is_arg_provided('--install-after') and isWorkingDir else json_config.get('installAfter', '')) or False)
  ...
  for lib in libraries:
    ...
    for deps_search_path in deps_search_paths:
      ...
      if p.exists(lib_path):
        ...
        lib_source = p.join(lib_path, lib_jxa_config.main_file) if lib_jxa_config.__dict__ else p.join(lib_path, 'index.js')
        ...
        if deps_search_path == deps_search_paths[-1]:
          print(f'Error, Could not find library "{lib}"')
          print('Make sure it\'s actually installed if using a package manager (e.g. "npm install")')
          exit(1)
  return dependencies
...
mirror_main_folder = p.join(tmp_dir, p.dirname(main_module.source).replace(working_dir + p.sep, ''))
...
try:
  ...
  exit(1)
main = CompilationUnit(mirror_main_source, p.join(output_dir, main_module.name))
libs: list[CompilationUnit] = []
for dep in dependency_modules:
  mirror_dependant_folder = p.join(tmp_dir, p.dirname(dep.dependant_source).replace(working_dir + p.sep, ''))
  mirror_dependant_source = p.join(mirror_dependant_folder, p.basename(dep.dependant_source))
  mirror_folder = p.join(tmp_dir, p.dirname(dep.source).replace(working_dir + p.sep, ''))
  ...
  try:
    ...
    print(f'Error, while copying dependency source to temporary directory: {e}')
    exit(1)
  try:
    ...
    exit(1)
  name_ver = dep.name + dep.version if re.match(r'.*\d+\.\d+\.\d+.*', dep.version) else dep.name
  ## Dots are not allowed in library names
  name_ver = name_ver.replace('.', '_')
  code = re.sub(f'(Library\\(["\'])(?:.+/)*{dep.name}(["\']\\))', f'\\g<1>{name_ver}\\g<2>', code)
  try:
...
bundle_executable = ''.join(word[0].capitalize() + word[1:] for word in re.split(r'[\s_-]+', p.basename(main.output_path)))
...
if jxa_config.deps_install_mode == 'app':
  if jxa_config.comp_mode != 'app':
    print('Error, library installation mode is set to "app", but the main file is not compiled to an app')
    exit(1)
  if jxa_config.install_after:
    main.output_path = p.join('/Applications', p.basename(main.output_path))
  lib_dest = p.join(main.output_path, 'Contents', 'Resources', 'Script Libraries')
  ...
  print(f'Error, unknown library installation mode "{jxa_config.deps_install_mode}"')
  exit(1)
...
if jxa_config.deps_comp_mode == 'script':
  ...
  print(f'Error, unknown library compilation mode "{jxa_config.deps_comp_mode}"')
  exit(1)
...
for item in [main] + libs:
  ...
  outputs[p.join(output_dir, p.basename(item.output_path))] = p.dirname(item.output_path)
  item.output_path = p.join(output_dir, p.basename(item.output_path))

  command = ['osacompile', '-l', 'JavaScript', '-o', item.output_path, item.input_path]
  try:
...
if jxa_config.comp_mode == "app":
  ...
  default_bundle_executable = re.search(r'(<key>CFBundleExecutable</key>\s+<string>)(\w+?)(</string>)', info_plist).group(2)

  ## Replace bundle executable name
  info_plist = re.sub(r'(<key>CFBundleExecutable</key>\s+<string>)\w+?(</string>)', f'\\g<1>{bundle_executable}\\g<2>', info_plist)

  ## Replace bundle icon name
  info_plist = re.sub(r'(<key>CFBundleIconFile</key>\s+<string>)\w+?(</string>)', f'\\g<1>{bundle_executable}.icns\\g<2>', info_plist)
  ...
  try:
    if p.exists(p.join(main.output_path, 'Contents', 'MacOS', default_bundle_executable)):
      os.rename(p.join(main.output_path, 'Contents', 'MacOS', default_bundle_executable), p.join(main.output_path, 'Contents', 'MacOS', bundle_executable))
  except Exception as e:
  ...
  try:
    os.rename(p.join(main.output_path, 'Contents', 'Resources', f'{default_bundle_executable}.icns'), p.join(main.output_path, 'Contents', 'Resources', f'{bundle_executable}.icns'))
    os.rename(p.join(main.output_path, 'Contents', 'Resources', f'{default_bundle_executable}.rsrc'), p.join(main.output_path, 'Contents', 'Resources', f'{bundle_executable}.rsrc'))
  except Exception as e:
  ...
  if jxa_config.app_icon != '':
    try:
      shutil.copy2(jxa_config.app_icon, p.join(main.output_path, 'Contents', 'Resources', f'{bundle_executable}.icns'))
    except Exception as e:

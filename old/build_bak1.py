...
class Literals:
  ...
  def __init__(self):
    self.working_dir = JxaConfLiteral('', '--working-dir')
    self.comp_mode = JxaConfLiteral('compilationMode', '--comp-mode')
    self.dep_comp_mode = JxaConfLiteral('dependencyCompilationMode',
                                        '--dep-comp-mode')
    self.dep_install_mode = JxaConfLiteral('dependencyInstallationMode',
                                           '--dep-install-mode')
    self.version = JxaConfLiteral('version', '--version')
    self.main_file = JxaConfLiteral('main', '--main-file')
    self.app_name = JxaConfLiteral('appName', '--app-name')
    self.app_icon = JxaConfLiteral('appIcon', '--app-icon')
    self.install_after = JxaConfLiteral('installAfter', '--install-after')
    self.package_name = JxaConfLiteral('packageName', '')
    self.only_dependencies = JxaConfLiteral('onlyDependencies',
                                            '--dependencies-only')
...
parser.add_argument('--version', type=str, help='Version of the project')
parser.add_argument('--main-file', type=str, help='Main source file')
parser.add_argument(
    '--app-name',
    type=str,
...
install_parser.add_argument(
    ...
    action='store_true',
    help=
    'Whether to install the app and it\'s dependencies too or only the dependencies'
)
...
class JxaConfig:
  ...
  def __init__(self, path: str):
    ...
    self.comp_mode: Literal['app', 'script', 'bundle'] = (
        (args.comp_mode if is_arg_provided('--comp-mode') and isWorkingDir else
         json_config.get('compilationMode', '')) or 'app').strip()
    self.dep_comp_mode: Literal['script', 'bundle'] = ((
        args.dep_comp_mode if is_arg_provided('--dep-comp-mode')
        and isWorkingDir else json_config.get('dependencyCompilationMode', ''))
                                                       or 'script').strip()
    self.dep_install_mode: Literal['app', 'user', 'system'] = (
        (args.dep_install_mode if is_arg_provided('--dep-install-mode') and
...
if jxa_config.dep_install_mode == 'app':
  if jxa_config.comp_mode != 'app':
    print(
  ...
  lib_dest = p.join(main.output_path, 'Contents', 'Resources',
                    'Script Libraries')
elif jxa_config.dep_install_mode == 'user':
  lib_dest = p.join(p.expanduser('~'), 'Library', 'Script Libraries')
elif jxa_config.dep_install_mode == "system":
  lib_dest = p.join('/Library', 'Script Libraries')
else:
  print(
      f'Error, unknown library installation mode "{jxa_config.dep_install_mode}"'
  )
  exit(1)

if jxa_config.dep_comp_mode == 'script':
  lib_ext = 'scpt'
elif jxa_config.dep_comp_mode == 'bundle':
  lib_ext = 'scptd'
else:
  print(
      f'Error, unknown library compilation mode "{jxa_config.dep_comp_mode}"')
  exit(1)

...
def build(**kwargs):
  ...
  try:
    ...
    log_print_error(f'Error, while deleting temporary directory: {e}')
    exit(1)
  try:
    shutil.rmtree(output_dir, ignore_errors=True)
  except Exception as e:
    log_print_error(f'Error, while deleting output directory: {e}')
    exit(1)
  ...
  try:
    ...
    log_print_error(f'Error, while creating temporary directory: {e}')
    exit(1)
  try:
  ...
  for dep in dependency_modules:
    ...
    try:
      ...
      log_print_error(f'Error, while reading dependant source: {e}')
      exit(1)
    ...
    try:
      ...
      log_print_error(f'Error, while writing dependant source: {e}')
      exit(1)
  ...
  if not p.exists(output_dir):
    try:
      ...
      log_print_error(f'Error, while creating output directory: {e}')
      exit(1)
  ...
  try:
    ...
    log_print_error(f'Error, while writing locations file: {e}')
    exit(1)
  ...
  if jxa_config.comp_mode == "app":
    try:
      ...
      log_print_error(f'Error, while reading Info.plist: {e}')
      exit(1)
    ...
    try:
      ...
      log_print_error(f'Error, while writing Info.plist: {e}')
      exit(1)
    ...
    try:
      ...
      log_print_error(f'Error, while renaming bundle executable: {e}')
      exit(1)
    ...
    try:
      ...
      log_print_error(f'Error, while renaming app resource files: {e}')
      exit(1)
    ...
    if jxa_config.app_icon:
      try:
        ...
        log_print_error(f'Error, while copying icon to the app resources: {e}')
        exit(1)
